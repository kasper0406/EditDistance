...
The string compression method used converts the input string into a restricted form of a context free grammar called a straight-line program (SLP). By utilizing a succinct representation of intermediate structures in the algorithm, it is possible to gain a theoretical speedup for compressible input.
...
It could be both of practical and theoretical interest to find a succinct representation of a SLP, in order to make the space consumption smaller. This would potentially make the SLP approach practical for long strings that do not compress very well.
...
It will now be examined how much of the running time is spent on the different parts of the algorithm for different input types and sizes. \Cref{fig:benchmark:relative-runningtime} shows how the relative time distribution is for the different input types.
