...
\paragraph{Availability:} All code, raw measurement data, plots and literature referred to and used in this thesis can be found at \thesisurl. A description of the structure of the code is also supplied on the website, together with a short description of how to build and run the code.
...
\end{itemize}
In order to gain a speedup the DIST repository is represented using a succinct representation. This representation encodes a DIST table as a permutation of the identity matrix and theoretically turns out to facilitate both fast construction and usage of the DIST repository.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[font=\tiny\sffamily,node distance=4.2cm]
    ...
    \node[block,fill=white!90!green, below=1cm of simple-classification](DIST-H-table-relation){
      \nodecontent{Relating of $\DIST$-tables and $H$-tables}
    ...
    \node[block,fill=white!90!green, left of=horizontal-merge](swap-DIST-strings){
      \nodecontent{Swap strings of $\DIST$ table}
  ...
  \caption[foo]{Overview of the proofs needed to describe and prove the correctness of the algorithm. Every box indicates a statement (claim, lemma or theorem) in this thesis. An edge between two boxes indicate a dependency between the statements. The boxes with a thick border indicates the main results needed for the implementation of the algorithm. The color coding of the boxes are assigned as follows:
  \begin{itemize}
    \coloredbox{white} No proof is given in this thesis.
    \coloredbox{white!90!cyan} No or only minor contribution in the proof or statement.
    ...
    \coloredbox{white!90!green} The statement is not mentioned or proved in the literature.
  \end{itemize}
  The diagram is intended to provide an easy to cope overview of how the results described in this thesis relate.
  }
  \label{fig:proof-overview}
\end{figure}

\section{Notation and definition of edit-distance}
A string $\str{A}{0}{m}$ denotes a sequence of $m$ characters with random access, where the first is inclusive and the last is exclusive. That is, for $i \in \{0, \dots, m - 1\}$ the $i$'th character in the string can be accessed by $A[i] \in \Gamma$. Throughout this report it is assumed that the alphabet $\Gamma$ is finite\footnote{In this thesis it has been chosen to use $\Gamma$ for the alphabet which strings are drawn from, since the $\Sigma$-symbol is used later to describe a matrix operation.}.
...
In order to test the algorithms, some test data need to be selected. As the algorithm relies on compression of the input, it is natural to choose test input with different compressibility characteristics. It is also worth noticing that the alphabet size does not directly influence the running time of the computations in the compression based algorithm. However, a larger alphabet will make the strings less compressible. Also, the algorithm compressing the strings relies on a suffix tree which have a $O(\log |\Gamma|)$ factor for following an edge. The compression algorithm however has a much better theoretical running time than the computation step, whereby this extra factor is negligible.
...
In order to verify the correctness of the implementation, the results of all tests has been checked up against the simple algorithm described in \cref{sec:intro:simple}. Also, unit tests have been made to individually test parts of the algorithm and during debugging and testing a lot of consistency checks are performed on every run. There are no known bugs in the implementation.
...
\begin{algorithm}[H]
  ...
  \Return $\SLP{A}$
  \caption{Algorithm for generating a $O(\log{n})$ approximation of the minimal SLP of the input string $\str{A}{0}{m}$.}
\end{algorithm}

When building the SLP it will be considered as an AVL-tree. Therefore it is known how to search and concatenate two SLPs in a way that keeps the resulting SLP balanced. The idea is to add a repeating factor in the string one at a time. The repeating factor is added by reusing the productions already in the SLP generating the factor. During the concatenating operation of SLPs, the resulting tree is balanced using appropriate AVL-tree rotations.

The $Concat$ process referenced in the code, is function almost the same way as concatenating two AVL trees. The only difference is that productions in the SLP may be shared, hence when adding nodes and making rotations, special care needs to be taken in order to ensure the balance of the tree and that the derived string is correct. This is easy to fix simply by introducing a constant number of new productions for every insertion and rotation. Amortized, the number of rotations that happen on every concatenation is constant and therefore only a constant number of extra productions are added every time $Concat$ is called.
...
First the suffix tree for the input string is generated. Then every node $v$ in the tree is annotated with the first index in the string where the substring spelled out from the root to $v$ occurs. This is done by traversing the tree bottom up. Any leaf is annotated by its suffix numbers and any non-leaf is annotated by the minimum over its children.
...
This section will examine how well different types of strings can be compressed to SLPs. It will also verified that the theoretical running time of $O(n\log{n})$ is satisfied in practice.
...
Therefore Fibonacci strings compress very well to a SLP, where a representation of $F_{i}$ uses only $i + 1$ productions for $i \geq 2$. It is noticed in \cref{claim:fib-slp-optimal} that there are no smaller grammar generating Fibonacci strings.
...
Therefore the quality of the compressed Fibonacci strings can be measured relative to the optimal SLP. \Cref{fig:compression:quality:fibonacci} shows a plot where the blue line shows how big a factor the computed SLP is bigger than the optimal SLP.
...
\Cref{fig:compression:runningtime} shows the normalized running time for constructing the SLP. From the plot it can be seen that the normalized running time for the different types of input more or less converges to a constant. For the longest input sizes especially the human genome test cases seems to increase. \Cref{fig:compression:instructions} shows that the normalized instruction count does not have this increase, but that the L2 cache misses increases. Therefore the extra increase can be explained by the hierarchical memory layout of a modern computer.
...
When it is clear which strings a $\DIST$- or $H$-table are based upon they may be omitted from the notation in order to simplify the look of the expressions.
...
\begin{claim}
  ...
  \begin{proof}
    ...
    \begin{align*}
      &\sum_{\substack{ \hat{i} \in \{i, \dots, n + m - 1\} \\ \hat{j} \in \{ 0, \dots, j - 1 \}}} P_{B,A}(m + n - \hat{i}, m + n - \hat{j}) \\
      = &\underbrace{(n + m)}_{\text{Entire matrix}} - \underbrace{(m + n - j)}_{\text{Left}} - \underbrace{((m + n) - (m + n - i))}_{\text{Right}} + \underbrace{P_{B,A}^{\Sigma}(m + n - i, m + n - j)}_{\text{Overlap}} \\
      = &j - i + P_{B,A}^{\Sigma}(m + n - i, m + n - j).
    \end{align*}
  \end{proof}
\end{claim}
...
\begin{align}
  ...
  &r_{lo}(j - i) = r_{hi}(j - i).
\end{align}
The first two cases can be checked by looping through all the $1$'s in both $P_{C,lo}$ and $P_{C,hi}$ and then for each of them check the corresponding path condition. Each check takes constant time and there are a total of $n$ $1$'s in the two matrices. The $1$'s from the first two cases can thereby be computed in time $O(n)$.
...
It will not be verified that the time to fill out the dynamical programming grid satisfies the theoretical running time. \Cref{fig:benchmark:fill-grid-time} plots the normalized running time for the different types of input. It can be seen that all the normalized running times seem to be bounded by a constant.
...
This section will present measurements on how the algorithm performs compared to the simple algorithm. \Cref{fig:benchmark:simple-vs-lcsblowup} plots the speedup factor obtained over the simple algorithm for the different inputs.
...
Since the alphabet is finite, the input strings can be made to compress arbitrarily well by making the strings sufficiently long. The rate of this compression happens by a factor of $O(\log_{|\Gamma|}{n})$ whereby the running time of the combined algorithm for all strings should be bounded by $O\left( N^2 \frac{\sqrt{\log{\log{N}}}}{\log{N}} \right)$. Since $O(\log{N})$ grows quite slow and the tremendous overhead in the compressed edit distance algorithm, the compression based algorithm will never be better than the simple algorithm for input that does not compress well under the SLP compression scheme.
