...
Another optimization for the simple algorithm is parallelization. However, both the simple algorithm and the main algorithm in this thesis \cite{Gawrychowski:2012:FAC:2422024.2422048} are easily parallelizable. In order to reduce the amount of code needed to implement the algorithms, none of them have has been parallelized. It is believed that the results presented in this thesis will generalize well to a parallelized version.

More detail on how to parallelize the compression based algorithm is given in \cref{sec:conclusion:further-work}.
...
The running time only depends on the compressed lengths of the strings, however it is easy to construct input strings such that the algorithm is a factor $n$ slower than the simple algorithm by exploiting the simplicity of the run-length encoding scheme.
...
The key to make the algorithm work, is to compress the input strings to what is known as straight-line programs (SLPs). A more formal definition of a SLP will be given in \cref{ch:compressing-strings}, for now it is sufficient to note that a SLP is a restricted context-free grammar.
...
When building the SLP, the SLP will be considered as an AVL-tree. Therefore it is known how to search and concatenate two SLPs in a way that keeps the resulting SLP balanced. The idea is to add add a repeating factor in the string one at a time. The repeating factor is added by reusing the productions already in the SLP generating the factor. During the concatenating operation of SLPs, the resulting tree is balanced using appropriate AVL-tree rotations.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=8cm]{distance-mult/max-dist-mult-cpu}
...
In \cref{sec:combining-the-two-steps} two factors called $ffactor$ and $xfactor$ was introduced. These factors controls how large each partition of the SLP should be. In this section these factors will be experimentally determined for use in all following benchmarks.

Notice that the input may have a significant influence on how these factors should be chosen. For finding a reasonable value for the factors, the combined running time of aligning two Fibonacci strings of length 610 and two uniform random strings of length 610. These strings represent two extremes for the SLP string compressibility (see \cref{sec:compression:benchmarks}). The choice is more or less arbitrary, but it is believed that factors offering the best compromise between these two strings are likely to yield a reasonable performance for most types of input.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=12cm]{parameter-test}
  \caption{Plot showing the combined running time of aligning two highly compressible sequences with each other and two poorly compressible sequences with each other.}
  \label{fig:benchmark:estimating-factors}
\end{figure}

\Cref{fig:benchmark:estimating-factors} shows a heat map of the combined running time. It can be seen that different choices of factors indeed have a big influence on the practical running time. The best running time seems to be obtained around $ffactor = 2$ and $xfactor = 0.7$. Therefore this choice has been chosen for all following benchmarks.
...
First the time taken to build the $\DIST$ repository will be examined. \Cref{fig:benchmark:dist-repo-time} shows the normalized running time for the different input types. For the Fibonacci input the normalized running time clearly seems to be bounded by a constant.
...
It will not be verified that the time to fill out the dynamical programming grid satisfies the theoretical running time. \Cref{fig:benchmark:fill-grid-time} plots the normalized running time for the different types of input. It can be seen that all the normalized running times except the onces for the longest Fibonacci inputs seem to be bounded by a constant. \Cref{fig:benchmark:fill-grid-cpu} shows a plot indicating that the number of CPU instructions executed follows the theoretical bound, but an increase in L3 cache misses is seen when the running time begins to increase. Therefore this increase in running time can be explained by the hierarchical memory layout of a computer.
...
It can be seen that the time for constructing and blocking the SLP takes almost all of the time for input sizes up to $\approx 2^7 = 128$. This is due to the high running time constant of building the suffix tree, used to compute the LZ-factorization. However, as the input size gets larger, the relative time for computing the SLP gets negligible in the total running time. This is also what is predicted by the theoretical running times.

For the Fibonacci input, it can be seen that the grid computation is most time consuming step of the algorithm. This is opposed to the three different inputs (the human genome combinations and the random string) where most of the time is spend computing the DIST repository. It may be that it is possible to tweak the block size parameter by a constant factor to make the time distribution more balanced, possibly reducing the constant behind the algorithm. A bit of experimentation of the block size parameter has been performed by multiplying the $f$-factor and $x$-factor from \cref{sec:combining-the-two-steps}. However, no combination has been found optimizing the running time for all the inputs, and in the benchmarks a $x$-factor of $1$ and $f$ factor of $2$ was used.
...
The situation is different for Fibonacci input, which is compressed such that $n = O(\log{N})$. This means that the total running time of the algorithm for Fibonacci input becomes $O\left( N\log{N} \sqrt{\log{\frac{N}{\log{N}}}} \right) = O(N\log{N}\sqrt{\log{N}})$. Therefore running time of the LCS Blow-up method should improve by a factor $O\left( \frac{N}{\log^{\frac{3}{2}}{N}} \right)$.

From \cref{fig:benchmark:simple-vs-lcsblowup} it can be seen that the speedup continues to improve as the length of the Fibonacci input grows. However, in order for the implementation to be faster than the simple algorithm, the length of the Fibonacci input should be longer than $\approx 2^{15} = 30000$.
