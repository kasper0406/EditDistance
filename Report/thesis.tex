...
In the previous step it was described how to select the productions for which the $\DIST$ repository should be build. This section will focus on how the $\DIST$ repository is constructed. More specifically the $\DIST$ repository contains a $\DIST$ table for every pair of selected productions in the two SLPs. In other words, let $P_\mathcal{A}$ and $P_\mathcal{B}$ denotes the selected productions in respectively the SLPs $\mathcal{A}$ and $\mathcal{B}$. Then the $\DIST$ repository contains a $\DIST$ table $\DIST_{a,b}$ for all $a \in P_\mathcal{A}, b \in P_{\mathcal{B}}$.
