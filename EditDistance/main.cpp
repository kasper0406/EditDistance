  ...
  for (uint64_t length = 1; length < 1000; length = max((uint64_t)((double)length * 1.2), length + 1)) {
    ...
    for (uint64_t x = 1; x < input.size(); x = max((uint64_t)((double)x * 1.2), x + 1)) {
      ...
      if (partitionString.str() != input) {
        cout << "Partition generation failed :(!" << endl;
      }
    }
  }
}
...
template <class SLPBuilder = SLP::SimpleCompressionSLPBuilder>
void test_dist_repository()
{
  for (uint64_t length = 10; length < 100; length = max((uint64_t)((double)length * 1.8), length + 1)) {
    ...
    for (uint64_t x = 3; x < min(A.length(), B.length()); x = max((uint64_t)((double)x * 1.3), x + 1)) {
      ...
      DIST::MergingDISTRepository<DIST::SimpleMerger> distRepo(*slpA, *slpB);
      distRepo.build(get<1>(partitionA), get<1>(partitionB));
      
      DIST::SimpleDISTRepository distRepo_(*slpA_, *slpB_);
      ...
      for (uint64_t i = 0; i < get<1>(partitionA).size(); ++i) {
        for (uint64_t j = 0; j < get<1>(partitionB).size(); ++j) {
          ...
          if (distRepo(a, b) != distRepo_(a_, b_)) {
            ...
            cout << "Oh no, they are different :(!" << endl;
          }
        }
      }
    }
...
void test_slp_compression_align() {
  ...
  if (aligner.edit_distance() != simple.edit_distance())
    cout << "Oh no, different results from computation of edit distance :(." << endl;
}

int main(int argc, char* argv[])
{
  test_partition_generation<SLP::SimpleSLPBuilder>();
  test_partition_generation<SLP::SimpleCompressionSLPBuilder>();
  test_horizontal_merge<DIST::SimpleMerger>();
  test_vertical_merge<DIST::SimpleMerger>();
  test_dist_repository();
  test_slp_compression_align<SLP::SimpleCompressionSLPBuilder, DIST::SimpleDISTRepository>();
  return 0;
